...
class ScheduleTeacherScreen extends ConsumerStatefulWidget {
  ...
  const ScheduleTeacherScreen({required this.account, super.key});

  @override
  ConsumerState createState() => _ScheduleTeacherScreenState();
}

class _ScheduleTeacherScreenState extends ConsumerState<ScheduleTeacherScreen>
    with HelperMixin {
  ...
  void _fetchAndMapSchedule(String teacherId) async {
    ...
    for (final classItem in response) {
      ...
      final startDate =
          DateTime.parse(classItem.startDate.toString()); // yyyy-MM-dd
  ...
  void initState() {
    super.initState();
    _fetchAndMapSchedule(widget.account!.teacher!.id.toString());
  }
              ...
              children: [
                ...
                Expanded(
                  child: events.isEmpty
                      ? const Center(child: Text('Không có lịch dạy.'))
                      : Padding(
                          ...
                          child: Scrollbar(
                            ...
                            child: ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final teacherName = (endParen != -1 &&
                                        endParen + 1 < event.length)
